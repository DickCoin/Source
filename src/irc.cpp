...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DickcoinTEST2\r");
            Send(hSocket, "WHO #DickcoinTEST2\r");
        } else {
            // randomly join #Dickcoin00-#Dickcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Dickcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Dickcoin%02d\r", channel_number).c_str());
        }
