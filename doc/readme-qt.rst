Dickcoin-qt: Qt4 GUI for Dickcoin
===============================
...
Alternatively, install Qt Creator and open the `Dickcoin-qt.pro` file.

An executable named `Dickcoin-qt` will be built.
...
A warning for people using the *static binary* version of Dickcoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Dickcoin is linked against libdb5.3.
...
installed causes Dickcoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
